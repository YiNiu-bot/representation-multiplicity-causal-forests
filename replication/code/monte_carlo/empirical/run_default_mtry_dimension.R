...
script_path <- if (length(script_flag)) {
  ...
  normalizePath("empirical/run_default_mtry_dimension.R")
}
...
source(file.path(project_dir, "empirical", "class_sampled_grf.R"))

threads <- as.integer(Sys.getenv("RI_THREADS", "15"))
replications <- as.integer(Sys.getenv("RI_DIM_REPS", "100"))
...
output_dir <- Sys.getenv(
  "RI_OUTPUT_DIR",
  file.path(project_dir, "empirical", "results_default_mtry_dimension")
)
...
prediction_losses <- function(prediction, truth) {
  treatment <- prediction > 0
  oracle <- truth > 0
  c(
    mse = mean((prediction - truth)^2),
    mae = mean(abs(prediction - truth)),
    mean_error = mean(prediction - truth),
    wrong_sign = mean(treatment != oracle),
    regret = mean(abs(truth) * (treatment != oracle)),
    value_gain = mean(truth * treatment),
    oracle_gain = mean(pmax(truth, 0)),
    treatment_share = mean(treatment)
  )
}
...
for (replication in seq_len(replications)) {
  ...
  for (dimension in dimensions) {
    ...
    predictions <- list(ordinary_x1 = x1, ordinary_x2 = x2)
    ...
    if (dimension == max(dimensions)) {
      ...
      predictions$class_x1 <- invariant_x1
      predictions$class_x2 <- invariant_x2
    ...
    stopifnot(all(vapply(predictions, function(z) {
      length(z) == n_test && all(is.finite(z))
    }, logical(1L))))
    for (method in names(predictions)) {
      losses <- prediction_losses(predictions[[method]], tau.test)
      loss_rows[[length(loss_rows) + 1L]] <- data.frame(
        replication = replication, dimension = dimension, method = method,
        metric = names(losses), value = unname(losses)
      )
    }
    saveRDS(list(
      data_seed = data_seed, forest_seed = forest_seed,
      dimension = dimension, truth = tau.test,
      target_modifiers = X.test[, seq_len(3L), drop = FALSE],
      predictions = predictions
    ), file.path(output_dir, sprintf("predictions_rep%03d_p%d.rds", replication, dimension)))
    gc(verbose = FALSE)
  }
}
...
write.csv(do.call(rbind, loss_rows), file.path(output_dir, "prediction_losses.csv"), row.names = FALSE)
