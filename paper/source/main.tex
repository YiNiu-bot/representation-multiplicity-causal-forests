...
recover the CATE. The default candidate rule yields a squared-depth threshold
for how quickly the representation size may grow. Monte Carlo and an Econometrica replication show
...
The candidate-count rule translates the access-depth index into a transparent
...
neighborhoods. Under the conditions studied here, consistency for each fixed
finite representation need not extend uniformly to growing collections of
information-equivalent covariates. Separate samples for split selection and
leaf estimation therefore leave an additional question about which
treatment-effect components the forest learns.

\citet{louppe2014} explains how retaining redundant predictors changes their
probability of entering the split search and can affect prediction. I study
the consequences of this mechanism for causal estimation and treatment
assignment. The representations considered here preserve both covariate
information and available threshold partitions, yet their growing
multiplicities can produce different limiting predictions. In the omission
regime, the two limits imply opposite treatment decisions for half the
population and persistent regret under one representation. Thus, a
preprocessing choice can have lasting consequences for treatment assignment
even when it adds no information.

The closest theoretical comparisons concern how feature subsampling and tree
depth govern recovery. \citet{chi2022high} and \citet{klusowski2024large}
...
prediction limits for sample-grown honest trees with data-dependent stopping.
Under the stated continuation and score-transfer conditions, these limits
determine which components are omitted or recovered. An additional
failure-path depth condition identifies the weight on a component under
incomplete recovery. Establishing these limits requires relating adaptive
splitting and stopping to the probability that a component is resolved before
leaf estimation. The squared-depth boundary under square-root candidate
sampling follows as a specialization.

The correction builds on existing approaches to group-aware feature sampling.
...
Proposition~\ref{prop:class} shows that sampling these classes, with canonical
routing and common class-level randomness, produces identical partitions,
weights, predictions, reported standard errors, and selected sets across
equivalent representations. The result gives an exact finite-array invariance
guarantee for a group-based correction, specifying the equivalence and routing
conditions needed to remove representation dependence.
...
\end{equation}
Conditional on the training data and fitted nuisance functions, the \(B_n\)
trees can be partitioned into independent groups of uniformly bounded size,
and every tree has the same marginal distribution.
\end{assumption}
...
to the forest's tuning parameters, consider the candidate-count law. According
to the \texttt{grf} 2.4.0 algorithm reference \citep{grfreference}, this law is
